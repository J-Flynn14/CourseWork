...
class ConnectFour:
    def __init__(self, ai_on: bool, agent:Any, n_cols:int, n_rows:int):
        ...
        self.init_window(n_cols, n_rows)
        self.init_menu()

    def init_window(self, n_cols, n_rows):
        pg.init()
    ...
    @staticmethod
    def do_move(board:np.ndarray, col:int, player:int) -> None:
        board = np.fliplr(np.rot90(board, 2))
        final_row = np.count_nonzero(board[:, col])

        if final_row != board.shape[0]:
            board[final_row, col] = player
    ...
    def step(self, action: int) -> np.ndarray:
        self.do_move(self.board, action, self.player)
    ...
    def get_action(self, observation:np.ndarray) -> int:
        ...
        if ((self.player == 1) or (self.player == 2 and not self.ai_on)) and self.b_down:
            ...
            action, score = self.agent.minimax(observation, self.depth, True)
